...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        templates = [
            {'trigger': 'pending', 'subject': 'Request Received: {event}', 'body': 'Dear {name},\n\nWe have received your booking request for {event} at {venue} on {date}. Your reference ID is {ref}.\n\nYour request is currently pending administrative review by the Ministry of Agriculture Leadership. You will be notified once the status is updated.'},
            {'trigger': 'management_approved', 'subject': 'Request Approved by MoA Management: {event}', 'body': 'Dear {name},\n\nYour booking request for {event} at {venue} on {date} has been reviewed and approved by Ministry of Agriculture Management.\n\nYour request has now been forwarded to the Event Management team for processing. Please note that your booking is not fully confirmed until payment is processed or further instructions are provided.'},
            {'trigger': 'management_rejected', 'subject': 'Booking Request Declined by MoA Management: {event}', 'body': 'Dear {name},\n\nWe regret to inform you that your booking request for {event} could not be approved by Ministry of Agriculture Management.\n\nReason Provided: {reason}\n\nWe apologize for any inconvenience. Please contact the MoA Events team if you need further clarification.'},
            {'trigger': 'partial_paid', 'subject': 'Advance Payment Confirmed: {event}', 'body': 'Dear {name},\n\nWe have successfully received your advance payment for {event} (Ref: {ref}). Your booking is now conditionally secured.\n\nPlease ensure the remaining balance is settled prior to the event.'},
